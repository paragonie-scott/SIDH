 ...
 */
CRYPTO_STATUS SIDH_curve_initialize(
...
bool is_CurveIsogenyStruct_null(PCurveIsogenyStruct pCurveIsogeny)
{
    ...
    if (
        pCurveIsogeny == NULL ||
        pCurveIsogeny->prime == NULL ||
        pCurveIsogeny->A == NULL ||
        pCurveIsogeny->C == NULL ||
        pCurveIsogeny->Aorder == NULL ||
        pCurveIsogeny->Border == NULL || 
        pCurveIsogeny->PA == NULL ||
        pCurveIsogeny->PB == NULL ||
 ...
 * can be used as private keys. It makes requests of random values with length 
 * "oAbits" (when AliceOrBob = 0) or "oBbits" (when AliceOrBob = 1) to the 
 * "random_bytes" function. The process repeats until random value is in
 * [0, Aorder-2]  ([0, Border-2], resp.). If successful, the output is given in
 * "random_digits" in the range [1, Aorder-1] ([1, Border-1], resp.).
 * The "random_bytes" function, which is passed through the curve isogeny 
 * structure PCurveIsogeny, should be set up in advance using SIDH_curve_initialize().
 * The caller is responsible of providing the "random_bytes" function passing random values as octets.
 */
CRYPTO_STATUS random_mod_order(
    digit_t* random_digits,
    ...
    if (
        random_digits == NULL ||
        is_CurveIsogenyStruct_null(pCurveIsogeny) ||
        AliceOrBob > 1
    ) {
        return CRYPTO_ERROR_INVALID_PARAMETER;
    }
    ...
    if (AliceOrBob == ALICE) {
        ...
        nbytes = (pCurveIsogeny->oAbits + 7)/8;
        ...
        // order/2:
        mp_shiftr1(order2, nwords);        
        // order2 = order/2-2:                    
        mp_sub(order2, t1, order2, nwords);
    } else {
        ...
        mask = 0x03;
        // order2 = order/3-2:
        mp_sub((digit_t*)Border_div3, t1, order2, nwords);
    }

    do {
        ntry++;
        // Max. 100 iterations to obtain random value in [0, order-2]:
        if (ntry > 100) {
        ...
        // Masking last byte :
        ((unsigned char*)random_digits)[nbytes-1] &= mask;
 ...
 * format that can be used as private keys to compute scalar multiplications 
 * using the elliptic curve BigMont. It makes requests of random values with 
 * length "BIGMONT_NBITS_ORDER" to the "random_bytes" function.
 * The process repeats until random value is in [0, BigMont_order-2] 
 * If successful, the output is given in "random_digits" in the range 
 * [1, BigMont_order-1].
 * The "random_bytes" function, which is passed through the curve isogeny 
 ...
 */
CRYPTO_STATUS random_BigMont_mod_order(
    digit_t* random_digits,
    PCurveIsogenyStruct pCurveIsogeny
) {
    ...
    // order2 = order-2:
    mp_sub(pCurveIsogeny->BigMont_order, t1, order2, nwords);
    // Value for masking last random byte: 
